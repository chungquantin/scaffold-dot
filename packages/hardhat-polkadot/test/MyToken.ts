...
describe("MyToken (Hardhat + Viem)", () => {
  ...
  beforeEach(async () => {
    ...
    const hash = await walletClient.deployContract({
      ...
      bytecode: (
        await hre.artifacts.readArtifact("MyToken")
  ...
  it("rejects minting by non-minters", async () => {
    ...
    await expect(() =>
      tokenAsAddr1.write.mint([addr2.account.address, amount])
    ).to.throw(/AccessControlUnauthorizedAccount/);
  });
  ...
  it("pauses transfers", async () => {
    ...
    await expect(() =>
      token.write.transfer([addr1.account.address, 1n])
    ).to.throw(/EnforcedPause/);
  });

  it("rejects pause by non-pauser", async () => {
    const tokenAsAddr1 = token.connect(addr1);
    await expect(() => tokenAsAddr1.write.pause()).to.throw(
  ...
  it("unpauses transfers", async () => {
    ...
    await expect(() =>
      token.write.transfer([addr1.account.address, 100n])
    ).not.to.throw();
  });
